...
export default {
    data() {
        return {
            // states : JSON.parse(localStorage.getItem('userData')) &&  JSON.parse(localStorage.getItem('userData')).role,
            menuItems: [],
    ...
    mounted: function () {
    var userDRoleData = JSON.parse(localStorage.getItem('userDetails')).access;
    const access = userDRoleData; 
    for(let i = 0; i < salesAdminMenuItems.length; i++){
        if(salesAdminMenuItems[i].access == null)
         this.menuItems.push(salesAdminMenuItems[i]);
        if(access.indexOf(salesAdminMenuItems[i].access) != -1 ){
           this.menuItems.push(salesAdminMenuItems[i]);
        }
    }
